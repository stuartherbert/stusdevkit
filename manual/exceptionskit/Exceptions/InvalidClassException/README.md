...
Thrown when a string is supplied as a class name, but that string does not identify a loadable class.
...
`InvalidClassException` is a fixed-shape problem-details exception for the "the caller passed a string that doesn't name a loadable class" case.

It hard-codes three of the RFC 9457 fields its parent normally accepts:

- **type** — `https://example.com/errors/invalid-class` _(placeholder — see [Here Be Dragons](#here-be-dragons))_
- **status** — `422` (Unprocessable Content)
- **title** — `"Class name does not refer to a loadable class"`

The caller supplies `$className` (the string that was offered as a class name). The constructor stores it in the parent's [`extra`](../Rfc9457ProblemDetailsException/getExtra.md) slot under the `class_name` key — not as a free-form [`detail`](../Rfc9457ProblemDetailsException/maybeGetDetail.md). The rest of the [`Rfc9457ProblemDetailsException`](../Rfc9457ProblemDetailsException/README.md) accessor surface (`getStatus()`, `getTitle()`, `getTypeAsString()`, `jsonSerialize()`, etc.) is inherited unchanged.
...
- [`->getExtra()`](../Rfc9457ProblemDetailsException/getExtra.md) — returns the extra data array (carries `['class_name' => $className]` for this class)
- [`->getStatus()`](../Rfc9457ProblemDetailsException/getStatus.md) — returns the HTTP status code (always `422` for this class)
- [`->getTitle()`](../Rfc9457ProblemDetailsException/getTitle.md) — returns the problem title (always `"Class name does not refer to a loadable class"` for this class)
...
**From InvalidClassException**

- [`->__construct()`](__construct.md) — build an `InvalidClassException` carrying the caller-supplied class name in the `extra` slot under the `class_name` key

## Here Be Dragons

**The wire-format key is `class_name`, not the PHP parameter name `className`.** The constructor accepts `$className` (camelCase), but the value lands in [`getExtra()`](../Rfc9457ProblemDetailsException/getExtra.md) under `class_name` (snake_case), to match the RFC 9457 convention for extension members. Callers reaching into `getExtra()` from a `catch` block must use the snake_case key — `getExtra()['className']` will silently return `null`.

**There is no `getClassName()` helper.** To recover the offending class name from a caught instance, read `$e->getExtra()['class_name']`.

**The `type` URI is a placeholder.** `https://example.com/errors/invalid-class` is not yet a real documentation URL — the docs target hasn't been set up. Treat it as a stable identifier, not a navigable link.
...
[`kits/exceptionskit/src/Exceptions/InvalidClassException.php:54`](../../../../kits/exceptionskit/src/Exceptions/InvalidClassException.php#L54)
...
- [Open issues mentioning `InvalidClassException`](https://github.com/stuartherbert/stusdevkit/issues?q=is%3Aissue+is%3Aopen+%22InvalidClassException%22)
- [Closed issues mentioning `InvalidClassException`](https://github.com/stuartherbert/stusdevkit/issues?q=is%3Aissue+is%3Aclosed+%22InvalidClassException%22)
