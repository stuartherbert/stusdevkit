...
Returns the first number stored in this list, or throws if the list is empty. This is the throwing counterpart to [`maybeFirst()`](maybeFirst.md). Use it when an empty list at this point in your code is a programming error rather than an expected branch — the exception names `ListOfNumbers` so the failure is easy to diagnose.
...
The first number stored in the list. The PHP signature returns `mixed`, but the class's `@template TValue of int|float = int|float` re-bound narrows this to `int|float`.
...
StusDevKit\CollectionsKit\Lists\ListOfNumbers
 ...
 ✔ Preserves int type for integer values
 ✔ Preserves float type for float values
```
...
- [`ListOfNumbers::maybeFirst()`](maybeFirst.md) — non-throwing variant for "may be empty" call sites
- [`ListOfNumbers::last()`](last.md) — last-number counterpart

## Issues

- [Open issues mentioning `ListOfNumbers::first()`](https://github.com/stuartherbert/stusdevkit/issues?q=is%3Aissue+is%3Aopen+%22ListOfNumbers%3A%3Afirst()%22)
- [Closed issues mentioning `ListOfNumbers::first()`](https://github.com/stuartherbert/stusdevkit/issues?q=is%3Aissue+is%3Aclosed+%22ListOfNumbers%3A%3Afirst()%22)
