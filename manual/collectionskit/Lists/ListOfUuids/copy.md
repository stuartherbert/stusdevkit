...
{
    /**
     * @return static<int, UuidInterface>
     */
    public function copy(): static
}
```
...
> The narrowed `static<int, UuidInterface>` return type shown above is bound by
> `@extends CollectionAsList<UuidInterface>` on [`ListOfUuids`](README.md).

## Description

Returns a new `ListOfUuids` containing the same UUIDs as this list. The two instances do not share storage — appending to or merging into the copy leaves the original unchanged, and vice versa.
...
Late-static binding ensures the copy is always an instance of `ListOfUuids`, not of an ancestor.
...
A new `ListOfUuids` instance containing the same UUIDs in the same order. The PHP signature returns `static`, but the class's `@extends CollectionAsList<UuidInterface>` binding narrows the value type to `UuidInterface`. Copying an empty list returns an empty `ListOfUuids`.
...
StusDevKit\CollectionsKit\Lists\ListOfUuids
 ✔ ->copy() returns a new ListOfUuids with the same data
...
- [Open issues mentioning `ListOfUuids::copy()`](https://github.com/stuartherbert/stusdevkit/issues?q=is%3Aissue+is%3Aopen+%22ListOfUuids%3A%3Acopy()%22)
- [Closed issues mentioning `ListOfUuids::copy()`](https://github.com/stuartherbert/stusdevkit/issues?q=is%3Aissue+is%3Aclosed+%22ListOfUuids%3A%3Acopy()%22)
