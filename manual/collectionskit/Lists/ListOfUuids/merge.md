...
{
    /**
     * @param AccessibleCollection<int, UuidInterface>|array<int, UuidInterface> $input
     * @return $this
...
> The narrowed `AccessibleCollection<int, UuidInterface>|array<int, UuidInterface>`
> parameter type shown above is bound by
> `@extends CollectionAsList<UuidInterface>` on [`ListOfUuids`](README.md).

## Description

Adds the UUIDs in `$input` to this list. `$input` may be a plain PHP array of UUIDs or another `ListOfUuids`.
...
This method modifies the receiving list — the UUIDs are appended, in order, after the existing entries. It does not return a copy.

The method returns `$this`, so calls can be chained fluently.

## Parameters

**`$input`** (`AccessibleCollection<int, UuidInterface>|array<int, UuidInterface>`)

The UUIDs to append. A plain PHP array is appended in its given order, and an `AccessibleCollection` argument must be a subtype of `ListOfUuids` (the late-static-bound class) — siblings and unrelated `AccessibleCollection` subclasses are rejected.

The PHP signature accepts `AccessibleCollection|array`, but the class's `@extends CollectionAsList<UuidInterface>` binding narrows the value type to `UuidInterface`.
...
- **[`InvalidArgumentException`](https://www.php.net/manual/en/class.invalidargumentexception.php)** — when `$input` is an `AccessibleCollection` that is not a subtype of `ListOfUuids` (propagated from [`mergeSelf()`](mergeSelf.md)).
...
StusDevKit\CollectionsKit\Lists\ListOfUuids
 ...
 ✔ ->merge() can merge another ListOfUuids
...
- [Open issues mentioning `ListOfUuids::merge()`](https://github.com/stuartherbert/stusdevkit/issues?q=is%3Aissue+is%3Aopen+%22ListOfUuids%3A%3Amerge()%22)
- [Closed issues mentioning `ListOfUuids::merge()`](https://github.com/stuartherbert/stusdevkit/issues?q=is%3Aissue+is%3Aclosed+%22ListOfUuids%3A%3Amerge()%22)
