...
{
    /**
     * @param array<int, UuidInterface> $data
...
Constructs a new `ListOfUuids`. When `$data` is supplied, every entry is stored as the list's initial contents, in the order given; sequential integer keys are preserved. When `$data` is omitted, the list starts empty.
...
**`$data`** (`array<int, UuidInterface>`, optional, default: `[]`)

The initial UUIDs to seed the list with. Each value must be a [`UuidInterface`](https://uuid.ramsey.dev/en/stable/) instance and must not be `null`. Keys are preserved as supplied, so callers can pass a list-shaped array or rely on the default of an empty list.
...
StusDevKit\CollectionsKit\Lists\ListOfUuids
 ...
 ✔ ::__construct() accepts initial UUIDs
...
- [`ListOfUuids::add()`](add.md) — append a single UUID after construction

## Issues

- [Open issues mentioning `ListOfUuids::__construct()`](https://github.com/stuartherbert/stusdevkit/issues?q=is%3Aissue+is%3Aopen+%22ListOfUuids%3A%3A__construct()%22)
- [Closed issues mentioning `ListOfUuids::__construct()`](https://github.com/stuartherbert/stusdevkit/issues?q=is%3Aissue+is%3Aclosed+%22ListOfUuids%3A%3A__construct()%22)
