...
Returns the last integer stored in this dict. Throws an exception if the dict is empty.

This is the throwing counterpart to [`DictOfIntegers::maybeLast()`](maybeLast.md). Use it when an empty dict at this point in your code is a programming error rather than an expected branch.
...
The last stored integer. The PHP return type is `mixed`; the class's template binding narrows it to `int`.
...
- [Open issues mentioning `DictOfIntegers::last()`](https://github.com/stuartherbert/stusdevkit/issues?q=is%3Aissue+is%3Aopen+%22DictOfIntegers::last()%22)
- [Closed issues mentioning `DictOfIntegers::last()`](https://github.com/stuartherbert/stusdevkit/issues?q=is%3Aissue+is%3Aclosed+%22DictOfIntegers::last()%22)
