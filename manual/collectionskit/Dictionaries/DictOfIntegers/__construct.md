...
> The narrowed `array<array-key, int>` shown above is bound by
> `@template-extends DictOfNumbers<array-key, int>` on [`DictOfIntegers`](README.md).

## Description

Creates a new `DictOfIntegers`, optionally seeded with an array of integers keyed by `array-key` (`int` or `string`).

The constructor accepts an associative array whose values are integers. The array is stored as the dict's initial contents.
...
**`$data`** (`array<array-key, int>`, optional, default: `[]`)

The initial contents of the dict. Keys are integers or strings; values must be integers. The PHP parameter type is `array`; the class's `@template-extends DictOfNumbers<array-key, int>` binding narrows it to `array<array-key, int>`.
...
- **[`NullValueNotAllowedException`](../../../exceptionskit/Exceptions/NullValueNotAllowedException/README.md)** — when any element in `$data` is `null`. The exception message names `DictOfIntegers` as the offending collection type.
...
- [Open issues mentioning `DictOfIntegers::__construct()`](https://github.com/stuartherbert/stusdevkit/issues?q=is%3Aissue+is%3Aopen+%22DictOfIntegers::__construct()%22)
- [Closed issues mentioning `DictOfIntegers::__construct()`](https://github.com/stuartherbert/stusdevkit/issues?q=is%3Aissue+is%3Aclosed+%22DictOfIntegers::__construct()%22)
