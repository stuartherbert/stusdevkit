...
> The narrowed `AccessibleCollection<array-key, float>` parameter type shown
> above is bound by `@template-extends DictOfNumbers<array-key, float>` on
...
Before any data is moved, this method calls the protected `canMerge()` helper to verify type compatibility. `canMerge()` accepts `$input` only when it is an instance of the calling class (resolved via late-static binding) or one of its subclasses. For `DictOfFloats`, that means any `DictOfFloats` or `DictOfFloats` subclass; sibling collections (e.g. `DictOfIntegers`) and unrelated `AccessibleCollection` subclasses are rejected with an `InvalidArgumentException`.

When the compatibility check passes, `$input`'s data is appended via PHP's array spread operator: entries with new keys are added, entries whose keys already exist overwrite the previously-stored float.

## Parameters

**`$input`** (`AccessibleCollection<array-key, float>`)

The source dict whose contents should be copied into this dict. Must be a `DictOfFloats` (or subclass); sibling collections and unrelated `AccessibleCollection` subclasses are rejected.
...
- **[`InvalidArgumentException`](https://www.php.net/manual/en/class.invalidargumentexception.php)** — when `$input` is an `AccessibleCollection` that is not a `DictOfFloats` (or subclass). The message is `type mismatch: cannot merge <input type> into DictOfFloats`.
...
- [`DictOfFloats::merge()`](merge.md) — adds the given input (array or compatible dict) to this dict
...
- [Open issues mentioning `DictOfFloats::mergeSelf()`](https://github.com/stuartherbert/stusdevkit/issues?q=is%3Aissue+is%3Aopen+%22DictOfFloats::mergeSelf()%22)
- [Closed issues mentioning `DictOfFloats::mergeSelf()`](https://github.com/stuartherbert/stusdevkit/issues?q=is%3Aissue+is%3Aclosed+%22DictOfFloats::mergeSelf()%22)
